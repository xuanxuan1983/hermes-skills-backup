...
## Variable Naming Conflicts with p5.js Built-ins

p5.js has many built-in functions and constants that will silently fail or produce wrong behavior if you name your variables the same thing:

```javascript
// BAD: 'scale' is a p5.js built-in function
let scale = 0.5;
scale(lerp(0.5, 1, scale));  // TypeError: scale is not a function

// GOOD: rename to avoid collision
let logoScale = 0.5;
scale(lerp(0.5, 1, logoScale));  // works correctly
```

**Known conflicts:**
- `scale` — p5.js transform function (2D/3D)
- `rotation`, `translation` — WEBGL mode
- `noise` — p5.js noise function
- `hue`, `saturation`, `brightness` — HSB color mode
- `red`, `green`, `blue` — color component accessors
- `alpha` — color alpha accessor
- `orbit` — potentially reserved in some contexts

**Rule: Always use descriptive, non-generic names.** Prefer `logoScale` over `scale`, `hueVal` over `hue`, etc.
