...
### x-browser.ts Fails → Fallback to opencli

If `x-browser.ts` fails with "Could not find the tweet composer" (X UI may have changed), use `opencli twitter post` as fallback:

```bash
BAOYU_CHROME_PROFILE_DIR=~/Library/Application\ Support/baoyu-skills/chrome-profile \
  opencli twitter post "Your tweet content"
```
...
If weibo-article reports "osascript paste permission denied" but shows "Content inserted via DataTransfer": the article body was successfully inserted. Browser stays open for manual review — proceed to publish manually.
