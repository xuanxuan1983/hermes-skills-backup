...
### weibo-post.ts / weibo-article.ts hangs (Chrome launch timeout)

These scripts use `launch({headless: false})` which opens a new Chrome window and hangs indefinitely waiting for it to "finish". 

**Fix**: Do NOT use the TypeScript scripts for posting. Instead use the opencli daemon command directly:

```bash
# Kill any stale Chrome instances first
pkill -f "remote-debugging-port.*baoyu-skills/chrome-profile" 2>/dev/null; sleep 2

# Then post via the running daemon
BAOYU_CHROME_PROFILE_DIR=~/Library/Application\ Support/baoyu-skills/chrome-profile opencli weibo post "Your post text"
```

The `opencli weibo post` command uses the already-running daemon and returns immediately. The scripts leave the browser open for manual review, but since they hang, use the daemon approach instead.

### Chrome debug port not ready

If `opencli weibo` commands fail with `Chrome debug port not ready` or `Unable to connect`, kill only the CDP Chrome instances, then retry:
...
### osascript paste fails → Content may still be inserted

If you see "osascript error: ...not allowed to send keystrokes" but the script output shows "Content inserted via DataTransfer" with a non-zero char count: the article body was successfully inserted. The browser remains open — just click publish manually in the browser window.
